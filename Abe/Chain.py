...
class X13Chain(Chain):
    def block_header_hash(chain, header):
        import x13_hash
        return x13_hash.getPoWHash(header)

class BadgerChain(X11Chain, PpcPosChain):
    def has_feature(chain, feature):
        return feature == 'nvc_proof_of_stake'
...
class Maru(X13Chain):
    def __init__(chain, **kwargs):
        chain.name = 'Maru'
        chain.code3 = 'MARU'
        chain.address_version = '\x2c'
        chain.script_addr_vers = '\x05'
        chain.magic = '\xfc\xc1\xb7\xde'
        Chain.__init__(chain, **kwargs)

    datadir_conf_file_name = "marucoin.conf"
    datadir_rpcport = 10038
	
class Badgercoin(BadgerChain):
    def __init__(chain, **kwargs):
        chain.name = 'Badgercoin'
        chain.code3 = 'BDG'
